...
G_ExitGesture(sendkey=false)
{
    ...
    if m_LastGestureKey in LButton
    {
...
UwpActive(title, ByRef hwnd)
{
    hwnd := 0
    WinGet _name, ProcessName, A
    if (_name = "ApplicationFrameHost.exe") {
        ControlGet _hwnd, Hwnd,, Windows.UI.Core.CoreWindow1, A
        if _hwnd {
            WinGet _name, ProcessName, ahk_id %_hwnd%
            if (_name = title) {
                hwnd := _hwnd
            }
        }
...
UwpSend(keys, hwnd)
{
    if G_GetFocusedControlClassNN() = "Windows.UI.Core.CoreWindow1"
        SendInput, %keys%
    else
